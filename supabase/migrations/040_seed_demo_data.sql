-- 040_seed_demo_data.sql
-- Single-transaction demo data seeder.
-- Replaces hundreds of individual REST API calls with one RPC.
...
BEGIN
  ...
  DELETE FROM campaign_events WHERE campaign_id IN (SELECT id FROM campaigns WHERE client_id = p_workspace_id);
  DELETE FROM campaign_jobs WHERE campaign_id IN (SELECT id FROM campaigns WHERE client_id = p_workspace_id);
  DELETE FROM campaigns WHERE client_id = p_workspace_id;
  DELETE FROM subscriber_list_memberships WHERE subscriber_id IN (SELECT id FROM subscribers WHERE client_id = p_workspace_id);
  DELETE FROM subscribers WHERE client_id = p_workspace_id;
  ...
  INSERT INTO subscribers (client_id, email, first_name, last_name, country, region, city, latitude, longitude, confirmed, unsubscribe_token, created_at)
  SELECT
    p_workspace_id,
  ...
  SELECT array_agg(id) INTO v_subscriber_ids FROM subscribers WHERE client_id = p_workspace_id;
  ...
  INSERT INTO campaigns (client_id, title, subject, audience, status, editor_html, editor_css, plain_text, sent_count, last_sent_at, created_at, updated_at)
  SELECT
    p_workspace_id,
    cd.title,
    cd.subject,
  ...
  FOR v_campaign IN
    SELECT id, sent_count, (v_now - last_sent_at) AS age
    FROM campaigns
    WHERE client_id = p_workspace_id AND status = 'sent'
  LOOP
    ...
    FOR v_hour IN 1..COALESCE(v_opened, 0) LOOP
      ...
      VALUES (
        ...
        'open',
        v_campaign.age + (v_day_offset * INTERVAL '1 day') + (INTERVAL '9 hours') + (random() * INTERVAL '8 hours')
      );
    END LOOP;
    ...
    FOR v_i IN 1..COALESCE(v_clicked, 0) LOOP
      v_sub := (SELECT id FROM unnest(v_subscriber_ids) id ORDER BY random() LIMIT 1);
      v_day_offset := floor(random() * 5)::INT;
      INSERT INTO campaign_events (campaign_id, subscriber_id, email, event_type, occurred_at, url)
      VALUES (
        v_campaign.id,
        v_sub.id,
        (SELECT email FROM subscribers WHERE id = v_sub.id),
