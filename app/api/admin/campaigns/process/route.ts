...
export async function POST(req: NextRequest) {
  ...
  for (const campaign of dueCampaigns) {
    ...
    if (geoFilter.regions.length > 0) recipientQuery = recipientQuery.in("region", geoFilter.regions);
    if (geoFilter.cities.length > 0) recipientQuery = recipientQuery.in("city", geoFilter.cities);
